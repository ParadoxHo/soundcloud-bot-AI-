...
# БЫСТРЫЕ настройки для скачивания по умолчанию
FAST_DOWNLOAD_OPTS = {
...
# МЕДЛЕННЫЕ настройки только для больших файлов (>50MB)
LARGE_FILE_OPTS = {
    'format': 'bestaudio[ext=m4a]/bestaudio[ext=mp3]/bestaudio/best',
...
class StableMusicBot:
    ...
    async def download_large_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        ...
        try:
            if status_message:
                await status_message.edit_text(f"⬇️ Скачиваем большой файл...\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
...
Выберите настройку для изменения:"""
    ...
    def run(self):
        print('🚀 Запуск оптимизированного Music Bot с поддержкой файлов до 200MB...')
        ...
        print('✅ Оптимизированный бот запущен! Маленькие файлы скачиваются быстро, большие - поддерживаются.')
        app.run_polling()
