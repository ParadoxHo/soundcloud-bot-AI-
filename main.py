...
MAX_FILE_SIZE_MB = 200  # Увеличили до 200MB

MAX_CONCURRENT_DOWNLOADS = 2  # Уменьшили для больших файлов
DOWNLOAD_TIMEOUT = 600  # Увеличили до 10 минут
SEARCH_TIMEOUT = 45     # Увеличили поиск

DYNAMIC_TIMEOUTS = {
    'short_track': 120,    # 2 минуты
    'medium_track': 300,   # 5 минут  
    'long_track': 480,     # 8 минут
    'very_long_track': 600, # 10 минут
    'search': 35
}

# Улучшенные настройки для скачивания больших файлов
SIMPLE_DOWNLOAD_OPTS = {
    ...
    'nooverwrites': False,
    'noplaylist': True,
    # УБРАЛИ ограничение размера файла
    'ignoreerrors': True,
    'ignore_no_formats_error': True,
    'socket_timeout': 30,
    'buffersize': 1048576,  # Увеличили буфер
    'http_chunk_size': 10485760,  # 10MB chunks для больших файлов
    'extractaudio': True,
    'audioformat': 'mp3',
    'audioquality': '0',
}
...
class StableMusicBot:
    ...
    def validate_track_fast(self, track_info: dict) -> bool:
        """Ультра-быстрая проверка трека БЕЗ ограничений по размеру"""
        ...
        # УБРАЛИ проверку на максимальную длительность
        if duration <= 10:  # Только минимальная проверка
            return False
            
        # Расширенный список проблемных ключевых слов
        ...
        return True  # Разрешаем все файлы независимо от размера

    def apply_user_filters(self, tracks: list, user_id: str) -> list:
        """Применяет все фильтры пользователя к списку треков"""
        if not tracks:
            return []
    ...
    async def check_file_size_before_download(self, url: str, track: dict) -> tuple:
        try:
            with yt_dlp.YoutubeDL(FAST_INFO_OPTS) as ydl:
                ...
                # ВСЕГДА разрешаем скачивание, независимо от размера
                can_download = True
            ...
            return 0, True  # Разрешаем скачивание даже при ошибке
    ...
    async def _handle_large_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, file_size: float):
        """Теперь просто информируем о размере, но не блокируем скачивание"""
    ...
    async def _find_audio_file(self, tmpdir: str) -> str:
        ...
        try:
            ...
            # Ищем файлы по размеру и расширению
            for file in files:
                filepath = os.path.join(tmpdir, file)
                
                # Проверяем размер (минимум 10KB)
    ...
    async def _send_audio_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE, 
        ...
        try:
            # Используем файловый объект для обхода ограничений
            with open(fpath, 'rb') as f:
                await context.bot.send_audio(
                    chat_id=update.effective_chat.id,
                    audio=f,  # Ключевое изменение - передаем файловый объект
    ...
    async def _pre_check_track(self, url: str, track: dict) -> bool:
        """Предварительная проверка трека перед скачиванием"""
        try:
    ...
    async def download_and_send_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        """Улучшенный метод скачивания с поддержкой больших файлов"""
        ...
        try:
            ...
            # Всегда разрешаем скачивание, но информируем о большом размере
            if file_size_mb > 50:
    ...
    async def simple_download(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        ...
        try:
            ...
            # ПРОВЕРЯЕМ НАЛИЧИЕ ФАЙЛОВ ДО ВСЕГО ОСТАЛЬНОГО
            files = os.listdir(tmpdir)
            if not files:
                logger.error(f"❌ Файлы не были скачаны для: {track.get('title')}")
                return False

            # Ищем любой файл с размером > 10KB
            audio_file = None
            for file in files:
                filepath = os.path.join(tmpdir, file)
                if os.path.getsize(filepath) > 10 * 1024:  # Минимум 10KB
                    audio_file = file
                    break
            ...
            # ИНФОРМИРУЕМ о размере, но НЕ БЛОКИРУЕМ
            ...
            # ДОБАВЛЯЕМ В ЧЕРНЫЙ СПИСОК ПРИ ОШИБКЕ
    ...
    async def search_soundcloud(self, query: str, album_only: bool = False, user_id: str = None):
        """Поиск на SoundCloud с применением фильтров пользователя"""
    ...
    async def random_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Поиск и скачивание случайного трека с применением фильтров"""
        user = update.effective_user
        self.ensure_user(user.id)
        ...
        try:
            # ПЕРЕДАЕМ user_id ДЛЯ ПРИМЕНЕНИЯ ФИЛЬТРОВ
...
Выберите настройку для изменения:"""
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обработчик текстовых сообщений с применением фильтров"""
    ...
    async def get_recommendations(self, user_id: str, limit: int = 30) -> list:
        ...
        # ПРИМЕНЯЕМ ФИЛЬТРЫ К РЕКОМЕНДАЦИЯМ
    ...
    async def get_popular_recommendations(self, limit: int = 15, user_id: str = None):
        """Быстрые популярные рекомендации с применением фильтров"""
        popular_tracks = []
    ...
    async def update_charts_cache(self, user_id: str = None):
        """Обновляет кэш чартов с применением фильтров"""
        now = datetime.now()
    ...
    async def show_charts(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Показывает топ чарты с применением фильтров"""
        user = update.effective_user
        self.ensure_user(user.id)
    ...
    async def generate_playlist(self, update: Update, context: ContextTypes.DEFAULT_TYPE, playlist_id: str):
        """Генерирует плейлист по шаблону с применением фильтров"""
        user = update.effective_user
        self.ensure_user(user.id)
    ...
    def run(self):
        print('🚀 Запуск улучшенного Music Bot с поддержкой файлов до 200MB...')
        ...
        print('✅ Улучшенный бот запущен и готов к работе с файлами до 200MB!')
        app.run_polling()
