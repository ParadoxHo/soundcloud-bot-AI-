...
# Улучшенные настройки для скачивания
SIMPLE_DOWNLOAD_OPTS = {
    ...
    'noprogress': True,
    'nopart': True,
    'nooverwrites': False,  # Разрешаем перезапись для избежания конфликтов
    'noplaylist': True,
    'max_filesize': MAX_FILE_SIZE_MB * 1024 * 1024,
    'ignoreerrors': True,
    'ignore_no_formats_error': True,
    'socket_timeout': 30,
    'extractaudio': True,  # Принудительно извлекаем аудио
    'audioformat': 'mp3',  # Конвертируем в MP3
...
class StableMusicBot:
    ...
    def validate_track_fast(self, track_info: dict) -> bool:
        ...
        # Дополнительная проверка для очень длинных названий (может быть проблемным)
        if len(title) > 100:
            return False
    ...
    async def _find_audio_file(self, tmpdir: str) -> str:
        """Улучшенный поиск аудио файлов"""
        try:
            files = os.listdir(tmpdir)
            logger.info(f"🔍 Поиск файлов в {tmpdir}: {files}")
            
            if not files:
                logger.error(f"❌ Директория пуста: {tmpdir}")
                return None
                
            # Расширенный список поддерживаемых форматов
            audio_extensions = ['.mp3', '.m4a', '.ogg', '.wav', '.flac', '.aac', '.opus', '.webm']
            
            # Сначала ищем по расширениям
            for file in files:
                file_ext = os.path.splitext(file)[1].lower()
                if file_ext in audio_extensions:
                    filepath = os.path.join(tmpdir, file)
                    # Проверяем размер файла
                    if os.path.getsize(filepath) > 1024:  # Минимум 1KB
                        logger.info(f"✅ Найден аудио файл: {file} ({os.path.getsize(filepath)} bytes)")
                        return file
            
            # Если не нашли по расширениям, ищем любой файл достаточного размера
            for file in files:
                filepath = os.path.join(tmpdir, file)
                if os.path.getsize(filepath) > 10 * 1024:  # Минимум 10KB
                    logger.info(f"⚠️ Используем файл без известного расширения: {file}")
                    return file
                    
            logger.error(f"❌ Не найдено подходящих файлов в: {files}")
            return None
            
        except Exception as e:
            logger.error(f"Ошибка поиска файлов в {tmpdir}: {e}")
            return None
    ...
    async def download_and_send_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        """Улучшенный метод скачивания с обработкой ошибок"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            logger.error("❌ URL трека отсутствует")
            return False

        # Дополнительная проверка перед скачиванием
        if not self.validate_track_fast(track):
            logger.error(f"❌ Трек не прошел валидацию: {track.get('title')}")
            if status_message:
                await status_message.edit_text(f"❌ Этот трек недоступен для скачивания\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
            return False
    ...
    async def simple_download(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        """Переработанный метод скачивания с улучшенной обработкой ошибок"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False
        ...
        try:
            ...
            # Улучшенные настройки скачивания
            ...
            def download_track():
                try:
                    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                        ...
                        if not info:
                            logger.error(f"Не удалось получить информацию о треке: {url}")
                            return None
                        
                        logger.info(f"Начинаем скачивание: {info.get('title', 'Unknown')}")
                        # Затем скачиваем
                        result = ydl.extract_info(url, download=True)
                        logger.info(f"Скачивание завершено. Файлы в {tmpdir}: {os.listdir(tmpdir)}")
                        return result
                except Exception as e:
            ...
            if not info:
                ...
                # Проверим, есть ли файлы в директории
                files = os.listdir(tmpdir)
                if files:
                    logger.info(f"Файлы найдены, но info=None: {files}")
                return False

            # Поиск скачанного файла
            audio_file = await self._find_audio_file(tmpdir)
            if not audio_file:
                logger.error(f"❌ Аудио файл не найден в: {os.listdir(tmpdir)}")
                return False
            ...
            if not os.path.exists(fpath):
                logger.error(f"Файл не существует: {fpath}")
                return False
                
            file_size = os.path.getsize(fpath)
            if file_size == 0:
                logger.error(f"Файл пустой: {fpath}")
                return False
    ...
    # ... (предыдущий код остается без изменений до метода recommendations_command)
