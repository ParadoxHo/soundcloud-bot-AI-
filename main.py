...
try:
    ...
    os.system("pip install python-telegram-bot yt-dlp psutil")
    try:
        from telegram import Update, InlineKeyboardButton, InlineKeyboardMarkup
        from telegram.ext import (
            Application, CommandHandler, MessageHandler, CallbackQueryHandler, filters, 
...
def check_memory_usage():
    """Проверяет использование памяти процессом"""
    try:
...
def set_memory_limits():
    ...
    try:
        ...
        new_soft = MEMORY_LIMIT_MB * 1024 * 1024
        resource.setrlimit(resource.RLIMIT_AS, (new_soft, hard))
        logger.info(f"✅ Лимит памяти установлен: {MEMORY_LIMIT_MB}MB")
    except Exception as e:
        logger.warning(f"Не удалось установить лимит памяти: {e}")

# Устанавливаем лимиты при запуске
set_memory_limits()
...
def get_cache_key(query: str, user_id: str) -> str:
    ...
    return hashlib.md5(f"{query}_{user_id}".encode()).hexdigest()

def get_cached_results(query: str, user_id: str):
...
def set_cached_results(query: str, user_id: str, results: list):
    """Сохраняет результаты в кэш"""
    key = get_cache_key(query, user_id)
    search_cache[key] = (results, time.time())
...
class ProgressTracker:
    ...
    async def update_progress(self, update: Update, file_size: int, downloaded: int, track_title: str):
        ...
        try:
            ...
            if query:
                ...
                # Для текстовых сообщений создаем новое
...
class RateLimiter:
    ...
    def check_rate_limit(self, user_id: str, action: str) -> Tuple[bool, str]:
        ...
        user_data['downloads'] = [t for t in user_data['downloads'] if t > hour_ago]
        user_data['searches'] = [t for t in user_data['searches'] if t > hour_ago]
        
        if action == 'download':
            if len(user_data['downloads']) >= RATE_LIMITS['downloads_per_hour']:
                next_reset = user_data['downloads'][0] + timedelta(hours=1)
                wait_minutes = (next_reset - now).seconds // 60
                return False, f"⚠️ Лимит скачиваний исчерпан. Попробуйте через {wait_minutes} мин."
            
            user_data['downloads'].append(now)
            return True, ""
            
        elif action == 'search':
            if len(user_data['searches']) >= RATE_LIMITS['searches_per_hour']:
                next_reset = user_data['searches'][0] + timedelta(hours=1)
                wait_minutes = (next_reset - now).seconds // 60
                return False, f"⚠️ Лимит поисков исчерпан. Попробуйте через {wait_minutes} мин."
            
            user_data['searches'].append(now)
            return True, ""
...
💾 Использование памяти: {memory_usage:.1f}MB
🗂️ Размер кэша поиска: {cache_size}
...
📊 Использование памяти ботом: {memory_usage:.1f}MB
📈 Всего памяти системы: {system_memory.total/(1024*1024):.1f}MB
📉 Использовано системой: {system_memory.percent}%
🗂️ Размер кэша поиска: {cache_size}
💿 Свободно на диске: {disk_usage.free/(1024*1024):.1f}MB
🔧 Лимит памяти бота: {MEMORY_LIMIT_MB}MB"""
...
async def admin_cache_clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await require_admin(update, context):
        return
    ...
    try:
        ...
        text = f"""🗂️ <b>Очистка кэша</b>

✅ Кэш поиска очищен
📊 Было записей: {old_size}
📈 Стало записей: {len(search_cache)}"""
...
/admin_memory - 💾 Информация о памяти
/admin_cache_clear - 🗂️ Очистка кэша
...
async def health_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        health_status = {
            ...
            'active_downloads': 0,  # Можно добавить отслеживание активных загрузок
            'cache_size': len(search_cache)
        }
...
💾 Память: {health_status['memory_usage_mb']}MB
👥 Пользователей: {health_status['user_count']}
🗂️ Кэш поиска: {health_status['cache_size']}
⏰ Время: {datetime.now().strftime('%H:%M:%S')}"""
        ...
        await update.message.reply_text("❌ Ошибка health check")
...
def setup_admin_commands(app):
    if ADMIN_IDS:
        ...
        app.add_handler(CommandHandler('admin_memory', admin_memory))
        app.add_handler(CommandHandler('admin_cache_clear', admin_cache_clear))
...
class StableMusicBot:
    ...
    async def _handle_large_file(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, file_size: float):
        ...
        text += f"⚠️ <b>Файл довольно большой, скачивание может занять время</b>\n\n"
    ...
    async def resilient_download(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None, max_retries: int = 3) -> bool:
        """Скачивание с повторными попытками при сетевых ошибках"""
        for attempt in range(max_retries):
            try:
                if attempt > 0 and status_message:
                    await status_message.edit_text(f"🔄 Повторная попытка скачивания ({attempt + 1}/{max_retries})...\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
                
                return await self.simple_download(update, context, track, status_message)
                
            except (ConnectionError, TimeoutError, asyncio.TimeoutError) as e:
                if attempt == max_retries - 1:
                    logger.error(f"Не удалось скачать после {max_retries} попыток: {e}")
                    if status_message:
                        await status_message.edit_text(f"❌ Ошибка сети после {max_retries} попыток\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
                    return False
                
                wait_time = 2 ** attempt  # Exponential backoff
                logger.info(f"Повторная попытка {attempt + 1} через {wait_time} сек...")
                await asyncio.sleep(wait_time)
                
            except Exception as e:
                logger.exception(f'Критическая ошибка скачивания: {e}')
                if status_message:
                    await status_message.edit_text(f"❌ Критическая ошибка\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
                return False
    ...
    async def download_and_send_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        """Улучшенный метод скачивания с поддержкой больших файлов и rate limiting"""
        user = update.effective_user
        
        # Проверка rate limit
        allowed, message = self.rate_limiter.check_rate_limit(str(user.id), 'download')
        if not allowed:
            if status_message:
        ...
        try:
            ...
            async with self.download_semaphore:
                ...
                return await asyncio.wait_for(
                    self.resilient_download(update, context, track, status_message),
                    timeout=timeout
                )
    ...
    async def simple_download(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict, status_message=None) -> bool:
        """Упрощенный и стабильный метод скачивания БЕЗ ограничений по размеру"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False
    ...
    # ==================== ПОИСК С ФИЛЬТРАМИ И КЭШИРОВАНИЕМ ====================

    async def search_music(self, query: str, album_only: bool = False, user_id: str = None):
        """Поиск музыки с кэшированием и применением фильтров"""
        ...
        async with self.search_semaphore:
            ...
            try:
                ...
                # Сохраняем в кэш
                set_cached_results(query, user_id, results)
            ...
            logger.info(f"✅ Найдено: {len(results)} отфильтрованных результатов для: '{query}'")
            return results
    ...
    async def search_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        # Проверка rate limit
        allowed, message = self.rate_limiter.check_rate_limit(str(user.id), 'search')
        if not allowed:
            await update.message.reply_text(message)
            return
    ...
    async def random_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            results = await self.search_music(random_search, user_id=str(user.id))
            if not results:
...
Выберите настройку для изменения:"""
    ...
    async def handle_text(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            results = await self.search_music(text, user_id=str(user.id))
            if not results:
    ...
    async def download_by_index(self, update: Update, context: ContextTypes.DEFAULT_TYPE, index: int, return_page: int = 0):
        ...
        # Создаем сообщение о статусе скачивания
        status_msg = await query.message.reply_text(f"⏬ Начинаем скачивание...\n🎵 {track.get('title', 'Неизвестный трек')[:30]}")
    ...
    async def get_popular_recommendations(self, limit: int = 15, user_id: str = None):
        ...
        for query in POPULAR_SEARCHES[:4]:
            try:
                results = await self.search_music(query, user_id=user_id)
                if results:
    ...
    async def download_from_recommendations(self, update: Update, context: ContextTypes.DEFAULT_TYPE, index: int):
        user = update.effective_user
        recommendations = user_data[str(user.id)].get('current_recommendations', [])
        current_page = user_data[str(user.id)].get('recommendations_page', 0)
        ...
        if success:
            ...
            await self.show_recommendations_page(update, context, current_page)
    ...
    async def download_from_charts(self, update: Update, context: ContextTypes.DEFAULT_TYPE, index: int):
        ...
        if success:
            ...
            await self.show_charts_page(update, context, current_page)
    ...
    async def generate_playlist(self, update: Update, context: ContextTypes.DEFAULT_TYPE, playlist_id: str):
        ...
        try:
            ...
            for query in playlist['queries'][:3]:
                try:
                    results = await self.search_music(query, user_id=str(user.id))
                    if results:
    ...
    async def download_from_playlist(self, update: Update, context: ContextTypes.DEFAULT_TYPE, index: int):
        ...
        if success:
            ...
            await self.show_playlist_page(update, context, current_page)

    def run(self):
        print('🚀 Запуск улучшенного Music Bot с мониторингом и кэшированием...')
        ...
        print(f'💾 Лимит памяти: {MEMORY_LIMIT_MB}MB')
        print(f'📊 Rate limiting: {RATE_LIMITS["downloads_per_hour"]} скачиваний/час')
        print(f'🔍 Кэширование поиска: {SEARCH_CACHE_TTL} сек')
