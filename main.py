...
# НОРМАЛЬНЫЕ НАСТРОЙКИ С КОНВЕРТАЦИЕЙ В MP3 (теперь с ffmpeg!)
ULTRA_FAST_DOWNLOAD_OPTS = {
    'format': 'bestaudio[filesize<45M]/bestaudio/best',
    'outtmpl': os.path.join(tempfile.gettempdir(), '%(id)s.%(ext)s'),
    'quiet': True,
    'no_warnings': True,
    
    # КОНВЕРТАЦИЯ В MP3 С FFMPEG
    'extractaudio': True,
    'audioformat': 'mp3',
    'audioquality': '5',
    
    # POSTPROCESSOR ДЛЯ КОНВЕРТАЦИИ
    'postprocessors': [{
    ...
    # ИСКЛЮЧАЕМ ПРОБЛЕМНЫЕ ФОРМАТЫ
    'noplaylist': True,
    
    'throttledratelimit': 31457280,
    'max_filesize': 47000000,
}

# БЫСТРЫЕ НАСТРОЙКИ ДЛЯ ПОЛУЧЕНИЯ ИНФОРМАЦИИ
ULTRA_FAST_INFO_OPTS = {
    'quiet': True,
    'no_warnings': True,
    'simulate': True,
...
class StableMusicBot:
    ...
    # ==================== ИСПРАВЛЕННЫЕ МЕТОДЫ СКАЧИВАНИЯ С FFMPEG ====================

    async def download_and_send_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """Основной метод скачивания с конвертацией в MP3"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False

        try:
            return await self.download_with_ffmpeg(update, context, track)
        except Exception as e:
            logger.exception(f'Ошибка скачивания трека: {e}')
            return False

    async def download_with_ffmpeg(self, update: Update, context: ContextTypes.DEFAULT_TYPE, track: dict) -> bool:
        """Скачивание с конвертацией в MP3 через ffmpeg"""
        url = track.get('webpage_url') or track.get('url')
        if not url:
            return False

        ydl_opts = ULTRA_FAST_DOWNLOAD_OPTS.copy()
        ...
        try:
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                # Ищем MP3 файл (после конвертации)
                for fname in os.listdir(tmpdir):
                    fpath = os.path.join(tmpdir, fname)
                    if os.path.isfile(fpath) and fname.lower().endswith('.mp3'):
                        ...
                        logger.info(f"✅ Трек успешно отправлен: {fname} ({actual_size_mb:.1f} MB)")
                        return True
            ...
            logger.exception(f'Ошибка скачивания с ffmpeg: {e}')
            return False

    async def get_file_size(self, url: str) -> float:
        """Получение размера файла"""
        try:
            with yt_dlp.YoutubeDL(ULTRA_FAST_INFO_OPTS) as ydl:
