...
class StableMusicBot:
    ...
    async def _recommendations_ready_notification(self, update: Update, context: ContextTypes.DEFAULT_TYPE, **kwargs):
        ...
        text += f"💡 Выберите трек для скачивания"
        ...
        try:
            ...
            if not recommendations:
                await self.send_smart_notification(
                    ...
                    query='ваши рекомендации', results_count=0, filtered_count=0
                )
                return
            ...
            await self.send_smart_notification(
                update, context, 'recommendations_ready',
            ...
            await self.send_smart_notification(
                update, context, 'download_error',
                track={'title': 'рекомендации'}, error_type='unknown'
            )
    ...
    async def show_charts(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not charts_data:
                await self.send_smart_notification(
                    update, context, 'download_error',
                    track={'title': 'чарты'}, error_type='download_failed'
                )
                return
            ...
            await self.show_charts_page(update, context, 0)
    ...
    async def show_charts_page(self, update: Update, context: ContextTypes.DEFAULT_TYPE, page: int):
        ...
        # Отправляем уведомление о готовности чартов
        await self.send_smart_notification(
            update, context, 'charts_ready',
    ...
    async def generate_playlist(self, update: Update, context: ContextTypes.DEFAULT_TYPE, playlist_id: str):
        ...
        try:
            ...
            if not all_tracks:
                await self.send_smart_notification(
                    update, context, 'search_results',
                    query=playlist['name'], results_count=0, filtered_count=0
                )
                return
            ...
            await self.show_playlist_page(update, context, 0)

        except Exception as e:
            logger.exception(f'Ошибка создания плейлиста: {e}')
            await self.send_smart_notification(
                update, context, 'download_error',
                track={'title': playlist['name']}, error_type='download_failed'
            )

    async def show_playlist_page(self, update: Update, context: ContextTypes.DEFAULT_TYPE, page: int):
        ...
        # Отправляем уведомление о готовности плейлиста
        await self.send_smart_notification(
            update, context, 'playlist_ready',
    ...
    async def random_track(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            if not results:
                await self.send_smart_notification(
                    update, context, 'search_results',
                    query=random_search, results_count=0, filtered_count=0
                )
                return
            ...
            if not filtered:
                await self.send_smart_notification(
                    update, context, 'search_results',
                    query=random_search, results_count=len(results), filtered_count=0
                )
                return
            ...
            await self.send_smart_notification(
                update, context, 'search_results',
                query=random_search, results_count=len(results), filtered_count=len(filtered)
            )
            ...
            await self.send_smart_notification(
                update, context, 'download_error',
                track={'title': 'случайный трек'}, error_type='download_failed'
            )
