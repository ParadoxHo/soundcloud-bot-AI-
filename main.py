...
GROUP_ID = os.environ.get('GROUP_ID')  # ID группы для работы
...
class GroupMusicBot:
    ...
    async def handle_all_messages(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Обрабатывает ВСЕ сообщения для диагностики"""
        try:
            print(f"🔍 ПОЛУЧЕНО СООБЩЕНИЕ:")
            print(f"   Чат ID: {update.effective_chat.id}")
            print(f"   Тип чата: {update.effective_chat.type}")
            print(f"   Юзер: {update.effective_user.first_name if update.effective_user else 'None'}")
            
            if update.message:
                print(f"   Текст: {update.message.text}")
                print(f"   ID сообщения: {update.message.message_id}")
            else:
            ...
            if update.effective_chat.id != GROUP_ID:
                print(f"   ⚠️ Игнорируем сообщение не из целевой группы")
                return

            message_text = update.message.text.strip().lower()
            user = update.effective_user
            
            print(f"   📨 Обрабатываем: '{message_text}' от {user.first_name}")
        ...
        try:
            ...
            if not query:
                await update.message.reply_text(
                    ...
                    f"💡 Пример: найди coldplay adventure"
                )
                return
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text(
            ...
            f"• <code>найди [запрос]</code> - найти трек по названию\n"
            f"• <code>рандом</code> - случайный трек\n\n"
            f"💡 <b>Примеры:</b>\n"
            f"<code>найди coldplay adventure of a lifetime</code>\n"
            f"<code>рандом</code>\n\n"
    ...
    def run(self):
        ...
        app.add_handler(MessageHandler(
            filters.ALL,  # ← ВИДИТ ВСЕ СООБЩЕНИЯ
        ...
        print('✅ Бот запущен!')
        print('💡 Теперь бот будет логировать ВСЕ сообщения')
        print('📋 Смотри логи в Railway для диагностики')
