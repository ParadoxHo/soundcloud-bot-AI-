...
class AIIntegration:
    ...
    async def generate_wish(self, user_query: str, track_title: str, artist: str = None) -> str:
        """Генерирует персонализированное пожелание"""
        if not self.enabled:
        ...
        try:
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    # Если вернул пустую строку или слишком длинный запрос, используем оригинальный
    ...
    def _build_wish_prompt(self, user_query: str, track_title: str, artist: str = None) -> str:
        """Создает промпт для генерации пожеланий"""
        base_prompt = f"""
...
"""
    ...
    def _build_search_prompt(self, original_query: str) -> str:
        """Создает промпт для улучшения поискового запроса"""
        prompt = f"""
...
class UniversalMusicBot:
    ...
    def clean_title(title: str) -> str:
        """Очищает название трека от лишней информации"""
        if not title:
        ...
        # Убираем упоминания uploader и другую ненужную информацию
        tags = [
    ...
    async def handle_find_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE, message_text: str):
        ...
        try:
            ...
            if self.ai.enabled:
                await status_msg.edit_text(f"{status_text}\n⏳ Этап 1/4: Уточняем запрос...", parse_mode='HTML')
                try:
                ...
                await status_msg.edit_text(f"{status_text}\n⏳ Этап 1/4: Анализируем запрос...", parse_mode='HTML')

            # Этап 2: Поиск
            await status_msg.edit_text(f"{status_text}\n⏳ Этап 2/4: Ищем лучший трек...", parse_mode='HTML')
            ...
            # Этап 3: Скачивание
            await status_msg.edit_text(
                f"{status_text}\n⏳ Этап 3/4: Скачиваем <b>{track.get('title', 'Неизвестный трек')}</b>...",
                parse_mode='HTML'
            )
            ...
            try:
                with open(file_path, 'rb') as audio_file:
                    ...
                    # Формируем подпись без упоминания ИИ
        ...
        async with self.search_semaphore:
            ...
            try:
                ...
                for entry in entries:
                    ...
                    # Если uploader есть в названии и это не похоже на имя исполнителя, пропускаем
                    if (uploader and 
    ...
    def _is_likely_artist(self, uploader: str, title: str) -> bool:
        """Проверяет, является ли uploader вероятным исполнителем"""
        # Если uploader короткий (2-3 слова) и встречается в начале названия - вероятно это исполнитель
        uploader_words = len(uploader.split())
        title_lower = title.lower()
        uploader_lower = uploader.lower()
        
        # Проверяем, находится ли uploader в начале названия
        if title_lower.startswith(uploader_lower):
            return True
        
        # Проверяем паттерны типа "Исполнитель - Название трека"
        if f"{uploader_lower} -" in title_lower or f"{uploader_lower} –" in title_lower:
            return True
            
        # Если uploader состоит из 1-3 слов, вероятно это исполнитель
    ...
    def run(self):
        ...
        print('📊 Поэтапный статус: уточнение запроса → поиск → скачивание → отправка')
